			...
			steps {
				script {
					...
					sshagent(["aws_server"]) {
						...
						sh "ssh -o StrictHostKeyChecking=no ${connectionToServer} export IMAGE_NAME=${IMAGE_TITLE} export USERNAME=${DOCKER_CREDENTIALS_USR} export PASSWORD=${DOCKER_CREDENTIALS_PSW}"
					}
				}
			}
